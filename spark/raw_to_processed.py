...
df = spark.read.json(f"gs://summoner-match/raw/{execution_date}/*.json").where(F.col('info.gameVersion') != '')
...
if args.match:
  ...
  match_df.withColumn('execution_date', F.to_date(F.lit(execution_date))) \
    .write.parquet(f"gs://summoner-match/processed/match/{execution_date}.parquet")
...
if args.champion:
  ...
  champ_df.withColumn('execution_date', F.to_date(F.lit(execution_date))) \
    .write.parquet(f"gs://summoner-match/processed/champion/{execution_date}.parquet")
