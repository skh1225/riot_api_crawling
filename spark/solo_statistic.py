...
for blob in blobs:
    ...
    if blob_split[0]=='bigquery' and blob_split[1]=='solo':
...
for i in range(1,15):
    ...
    if d in date_set:
        file_list.append(f"gs://gnimty_bucket/bigquery/solo/{d}.parquet")
...
for i in range(len(tier_list)):
    ...
    .option("database","statistics").option("collection", f"champion_statistics_solo_{tier_list[i]}").save()
