...
match_df = spark.read.schema(schema).parquet(f"gs://gnimty_bucket/raw/solo/{execution_date}/*.parquet")
...
match_df.select(struct_cols) \
    ...
    .parquet(f"gs://gnimty_bucket/bigquery/solo/{execution_date}.parquet")

# match_df.select(struct_cols).write.format('bigquery') \
#   .option('table', 'match_summary.solo') \
#   .option("temporaryGcsBucket","gnimty_bucket/bigquery")\
#   .mode("append") \
#   .save()